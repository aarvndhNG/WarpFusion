...
# Launch ngrok to expose the local Jupyter server
echo "Launching ngrok to expose the local Jupyter server..."
ngrok http 8888 --log=stdout > ngrok.log &
NGROK_PID=$!

# Wait for ngrok to start and parse the ngrok URL
sleep 10
NGROK_URL=$(grep -o "http://[a-z0-9]*\.ngrok.io" ngrok.log)
...
echo "------"
jupyter notebook  --allow-root --ServerApp.open_browser=False --no-browser --ServerApp.allow_remote_access=True --ServerApp.allow_origin='${NGROK_URL}' --port=8888 --ServerApp.port_retries=0 --ip=0.0.0.0
